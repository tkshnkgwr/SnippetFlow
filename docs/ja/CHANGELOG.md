...
- **総合ヘルプ・使い方ガイドの大幅拡充 (`src/App.tsx`)**:
  - ヘルプモーダルを3タブ構成（「💡 活用シーン・基本」「🛠️ 各機能の詳細」「⌨️ 操作 & システム」）へ刷新。
  - 実務で役立つ4大活用シーン（AIプロンプトの保存、ビジネスメール・チャット定型文、定型コマンド・スクリプトの保管、一時メモ・クイック保存）をカードUIで提示。
  - 差分比較（LCSアルゴリズム・ユースケース・操作方法）、複数結合（用途・区切り文字・順序調整）、性能メーター（累計短縮時間・ベンチマーク・大量負荷テスト）の機能ディープダイブ解説を新設。
- **各種ドキュメントの全面見直し・拡充**:
  - `README.md`, `README_JA.md`: 「💡 こんな用途に！おすすめの活用シーン」および主要機能一覧の拡充。
  - `docs/ja/SPEC.md`, `docs/en/SPEC.md`: 「4.6. ヘルプ・使い方ガイド (Help Guide)」の仕様新設。
  - `docs/ja/USER_GUIDE.md`, `docs/en/USER_GUIDE.md`: 活用シーン・各機能の詳細解説を全面同期。
  - `docs/ja/TODO.md`, `docs/en/TODO.md`: 実装済みタスクの最新化。
- **RustDoc / TypeScriptDoc の完全整備 (`src-tauri/src/lib.rs`, `src/types.ts`)**:
  - RustDoc クレート概要および intra-doc リンク記法（`[`models`][]` 形式）への適合（`-D warnings` 警告ゼロ）。
...
### Added
- **複数タグ選択時のAND条件絞り込みへの対応 (`src/components/SnippetList.tsx`, `src-tauri/src/lib.rs`)**:
  - タグ検索時に複数のタグを選択した際、選択したすべてのタグを含むスニペットのみを表示するAND条件フィルタリングに対応。

### Changed
- **Tauri専用構成へのプロジェクト構造・フォルダ構成の最適化 (`src/`, `Cargo.toml`, `vite.config.ts`, `index.html`, `tsconfig.json`)**:
  - 使用しなくなった純Rust/egui版（`src-egui/`）のソースコードおよび関連設定を削除除外。
  - フロントエンドディレクトリを `src-react/` から標準の `src/` へリネームし、Tauri単体アプリケーションとして構成を最適化。全ドキュメントを同期更新。
...
### Added
- **暗号化保存のデフォルト有効化 (`src-react/hooks/useSnippets.ts`, `src-egui/storage.rs`)**:
  - Tauri版および egui版の双方において、定型文のローカル保存モードをデフォルトで暗号化保存（`ENC1:` ヘッダー方式）へ有効化。保存ファイルのセキュリティと不可読性を向上。

## [1.13.2] - 2026-07-23

### Fixed
- **新規定型文保存時のデシリアライズ失敗エラーの修正 (`src-tauri/src/lib.rs`, `src-react/hooks/useSnippets.ts`)**:
  - `TauriSnippet` 構造体のオプショナルフィールドに `#[serde(default)]` を適用し、キーが欠落したオブジェクト受信時のデシリアライズ拒否エラーを解決。
  - フロントエンドでの新規定型文生成時に初期プロパティ値を明示設定し、保存処理の完全動作を保証。
