...
`SnippetFlow` is a cross-platform snippet manager built using Tauri v2 (React 19 / TypeScript + Rust). 
...
- **Data Persistence and Model Validation**:
  ...
  - Encrypted (`ENC1:` header) and plain-text safe loading/saving mechanisms.
...
1. **Add Unit Tests**:
   - If you modify core logic (`common_lib`) or state management models, you must add or expand unit tests in `src-tauri/src/` or the tests module within `common_lib/src/`.
2. **Crash-Free Design Testing**:
   - Maintain tests that verify the app safely falls back to default data instead of panicking if `snippets.json` is corrupted or missing.
